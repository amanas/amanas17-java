...
public class ExhaustiveSearch extends ASSearch implements OptionHandler {
	...
	public Enumeration listOptions() {
		...
		newVector.addElement(
				new Option("\tOutput subsets as the search progresses." + "\n\t(default = false).", "V", 0, "-V"));
		...
		while (!done) {
			...
			if (tempMerit >= best_merit) {
				...
				if (tempMerit > best_merit || (tempSize < sizeOfBest)) {
					...
					sizeOfBest = tempSize;
					if (m_verbose) {
						System.out.println("New best subset (" + Utils.doubleToString(Math.abs(best_merit), 8, 5)
	 ...
	 * 
	 * @param size
	 *            the size of the feature subset (eg. 2 means that the current
	 *            subset contains two features and the next generated subset
	 *            should also contain 2 features).
	 * @param temp
	 *            will hold the generated subset as a BitSet
	 */
