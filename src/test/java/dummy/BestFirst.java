...
public class BestFirst extends ASSearch implements OptionHandler, StartSetHandler {
	...
	public int[] search(ASEvaluation ASEval, Instances data) throws Exception {
		...
		BitSet best_group, temp_group=null;
		int stale;
		double best_merit;
		double merit;
		boolean z;
		...
		while (stale < m_maxStale) {
			...
			do {
				...
				for (i = 0; i < m_numAttribs; i++) {
					...
					if (z) {
						...
						/*
						 * if this subset has been seen before, then it is
						 * already in the list (or has been fully expanded)
						 */
			...
			/*
			 * if we haven't added a new attribute subset then full expansion of
			 * this node hasen't resulted in anything better
			 */
			if (!added) {
				stale++;
			}
